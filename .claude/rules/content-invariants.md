...
- **INV-6: No `\pause` or overlays.** Beamer `\pause`, `\only`, `\visible`, `\onslide` commands are forbidden in any `.tex` slide source. For Quarto decks, do not use `::: {.incremental}` wrappers either — they break the watch-along format.
